...
export const STORAGE_KEY = 'tracknamic-sandbox-prompts';
export const DEFAULT_PROMPTS = [
  {
    id: 'sample-1',
    title: 'Friendly customer support reply',
    body: '# Persona\nYou are a concise, friendly support agent.\n\n# Task\nDraft a response that clarifies the issue and proposes a next step.',
    tags: ['support', 'tone', 'concise'],
    model: 'gpt-4o-mini',
    temperature: '0.30',
    savedAt: Date.now(),
  },
  {
    id: 'sample-2',
    title: 'Technical explainer for engineers',
    body: '# Persona\nYou are a senior platform engineer.\n\n# Task\nExplain feature flags and rollout safety with crisp bullets and examples.',
    tags: ['engineering', 'explain', 'flags'],
    model: 'gpt-4o',
    temperature: '0.20',
    savedAt: Date.now(),
  },
...
export function persistPrompts() {
...
export function setPrompts(nextPrompts) {
  prompts = nextPrompts;
}
...
function focusSandbox() {
  ...
  setTimeout(() => {
    document.querySelector('#prompt-form input[name="title"]').focus();
  }, 250);
}

function setupLaunchSandbox() {
  const triggers = document.querySelectorAll('[data-launch-sandbox]');
  triggers.forEach((trigger) => {
    trigger.addEventListener('click', (event) => {
      event.preventDefault();
      focusSandbox();
      const seeded = seedDefaultsIfEmpty();
      if (seeded) {
        renderPromptList();
        setActivePromptLabel();
      }
...
export function bootstrapApp() {
  const rootExists = document.getElementById('module-grid');
...
if (typeof document !== 'undefined') {
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    requestAnimationFrame(bootstrapApp);
  } else {
